...
**Phase 3 (Critique):** Using the verification data you already gathered (do not re-verify), evaluate the plan against checklist criteria 1, 2, 4, 7, 8, 12, AND the manual-deploy / autonomy-violations / manifest-coherence / mid-flow-human-review rows of criterion 10 through your accuracy-and-fidelity lens. Tag every finding with [Verifier].
...
- **Criterion 10 — autonomy-violations row:** Walk every Task body in the plan, scanning for the signals listed under "Autonomy violations — signal list" in the checklist (paid API calls, Dashboard/UI work, OAuth consent, manual paste, bare full-suite test runners without a file path, `npm run build` / `next build` / `tsc` whole-project / `npm run lint` whole-codebase, tasks titled "Final verification" / "Full-suite verification" / "End-to-end verification" or any equivalent). Any match inside a Task block (rather than `## Prerequisites` or `## Manual Steps (Post-Automation)`) is **high** severity — cite the task number and exact step text.
...
- **Criterion 12 — buildability:** Walk every `Create:` / `Modify:` code block in the plan. (a) For `app/**/page.tsx` / `layout.tsx` paths, list each named export and diff against the whitelist enumerated in criterion 12 of the checklist — any non-whitelisted export is HIGH severity. (b) For snippets beginning with `'use client'`, check their import specifiers against the checklist's fs-using engine-module list — any match is HIGH severity. (c) For tasks that change the arguments passed to a function the plan's test specs `jest.mock`, confirm the same task updates the function's declared parameter type — a call-site change without the co-located declaration update is HIGH severity. Cite the task number and exact snippet line for every finding.
