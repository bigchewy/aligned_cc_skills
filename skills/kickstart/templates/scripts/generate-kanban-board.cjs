...
//
// Run: node scripts/generate-kanban-board.js
...
When you're done I'll regenerate the board (node scripts/generate-kanban-board.js) and triage the survivors.`;
...
<body>
  ...
  <div class="prompt-card action">
    <div class="label">
      ...
      <p class="hint">Each Resolve item is gated behind an explicit <code>/aligned:root-cause-analysis</code> step. The prompt refuses to do work without RCA validating it first.</p>
    </div>
    <div class="actions">
  ...
  <footer class="page-footer">
    ...
    <p><strong>Regenerate:</strong> <code>node scripts/generate-kanban-board.js</code></p>
  </footer>
  ...
  function buildPrompt() {
    ...
    let out = 'Work through the following Kanban items. Do NOT assume any item is correct just because it appears below — every Resolve item MUST pass a root-cause check before any code is touched.\\n\\n';
    out += 'Mandatory order of operations:\\n';
    out += '  1) For each Resolve item, invoke /aligned:root-cause-analysis on its Observed problem.\\n';
    out += '  2) Only if RCA confirms the issue is real, present, and the proposed solution is sound — then implement.\\n';
    out += '  3) If RCA refutes the item (issue gone, solution wrong, etc.), do not implement. Move the card to docs/kanban/done/ or docs/kanban/did_not_complete/ with a one-line note explaining what RCA found.\\n\\n';

    if (resolve.length > 0) {
      out += '## Resolve (RCA-gated — do not skip step 1)\\n\\n';
      for (const i of resolve) {
        out += '- ' + i.file + ' — ' + i.title + ' [' + i.severity + ']\\n';
        if (i.location) out += '  Location: ' + i.location + '\\n';
      }
      out += '\\nFor each Resolve item, the full sequence is:\\n';
      out += '  a) Read the full markdown for context.\\n';
      out += '  b) Run /aligned:root-cause-analysis on the Observed problem.\\n';
      out += '  c) Report back: did RCA validate or refute the item? Quote the RCA conclusion.\\n';
      out += '  d) If validated → implement the fix, add or update tests, then move the markdown from docs/kanban/todo/ to docs/kanban/done/ with a one-line commit reference.\\n';
      out += '  e) If refuted → move the markdown to docs/kanban/done/ (resolved elsewhere) or did_not_complete/ (invalid) with the RCA reason.\\n';
      out += 'Pause and ask before proceeding if any RCA result is ambiguous.\\n\\n';
    }
    ...
    out += 'After finishing, regenerate the board: node scripts/generate-kanban-board.js\\n';
    return out;
  }
