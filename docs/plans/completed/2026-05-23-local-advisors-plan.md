...
#### Post-execution correction: actual post-migration count is 58 advisors, not 57
This plan was written expecting a 57-advisor steady state (Task 15 Step 5, Task 16, Task 17, Manual Steps). The actual shipped count is **58**. Cause: Task 10's new `test_framework_advisor_references.py` guard immediately surfaced a pre-existing dangling reference — `storybrand-homepage` referenced `donald-miller`, who had no `advisors/registry.yaml` entry and no prompt file on `main`. The fix added the missing advisor (`advisors/prompts/donald-miller.md` + a registry entry), raising the pre-migration count from 76 to 77 and the post-migration count from 57 to 58. All shipped artifacts (`plugin.json`, `marketplace.json`, `advisors/README.md`, `frameworks/README.md`, `docs/advisor-catalog.html`) correctly reflect **58 advisors / 112 frameworks**; the `57` figures elsewhere in this plan are the original (now superseded) estimate.
